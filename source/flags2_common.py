...
SERVERS = {
    ...
    'LOCAL': 'http://localhost:8001/data/flags',
    'DELAY': 'http://localhost:8002/data/flags',
    'ERROR': 'http://localhost:8003/data/flags',
    'TESSA': 'http://localhost:8000/static/flags',
}
...
"""
Этот модуль содержит общие функции и параметры, используемые 
во всех `flags2`-скриптах, в том числе функция `main`, которая 
занимается разбором коммандной строки, хронометражем и выводом 
результатов.
"""
...
def final_report(cc_list, counter, start_time):
    ...
    # Elapsed time - (анг.) пройденное время.
    ...
    if every_cc:
        ...
        for cc in (c.upper() for c in cc_args):
            if len(cc) == 1 and cc in A_Z:
                ...
                msg = 'each CC arguments must be A to Z or AA to ZZ.'
